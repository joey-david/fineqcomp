...
\begin{itemize}
  ...
  \paperidea{Explain the old 2-bit code with an explicit zero level and why the four-codeword geometry differs from the sign-preserving four-level code.}
...
\begin{itemize}
  ...
  \paperidea{Old 2-bit zero-containing representation collapses despite nearly matching the sign-only file rate; use this as evidence that ``number of bits'' alone does not characterize a code.}
...
\begin{itemize}
  \paperidea{Each LoRA row chooses among: omit it entirely, or encode it with 1/2/3/4/8-bit sign-preserving quantization.}
